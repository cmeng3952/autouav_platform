...
public:
    ArucoTracker() : nh_("~"), 
                     ...
                     lidar_height_(0.13), lidar_available_(true), last_lidar_time_(0),
                     height_source_priority_(1), lidar_enable_(true), 
                     lidar_timeout_(1.0), lidar_noise_threshold_(0.01), lidar_fusion_weight_(1.0),
                     ...
                     // 风扰动补偿相关初始化
                     wind_compensation_enable_(true), position_filter_x_(), position_filter_y_(),
                     velocity_filter_x_(), velocity_filter_y_(), wind_estimation_filter_x_(), wind_estimation_filter_y_(),
                     wind_compensation_x_(0.0), wind_compensation_y_(0.0),
                     // 四角标记相关初始化
    ...
    void relAltCallback(const std_msgs::Float64::ConstPtr& msg) {
        ...
        if (enable_lidar_debug_ && (ros::Time::now() - last_height_print).toSec() > height_print_interval_) {
            ...
            ROS_INFO("Height comparison: rel_alt=%.2fm, local_z=%.2fm, lidar=%.2fm, aruco=%.2fm, fused=%.2fm", 
                     rel_alt_, odom_z, lidar_height_, aruco_height, getCurrentHeight());
        ...
        if(!msg->targets.empty()){
            ...
            if(target_count > 0){
                ...
                if (enable_aruco_depth_debug_) {
                    ...
                    if ((ros::Time::now() - last_depth_print).toSec() > 2.0) {
                        double aruco_height = getArucoHeight();
                        if (aruco_height > 0) {
                            std::string depth_info = "ArUco depths: ";
                            if (current_has_19 && first_19_idx >= 0) {
                                depth_info += "ID19=" + std::to_string(aruco_depth_[first_19_idx]) + "m ";
                            }
                            if (current_has_43 && first_43_idx >= 0) {
                                depth_info += "ID43=" + std::to_string(aruco_depth_[first_43_idx]) + "m ";
                            }
                            depth_info += "-> Height=" + std::to_string(aruco_height) + "m";
                            ROS_INFO("%s", depth_info.c_str());
                        }
...
private:
    ...
    void loadParams() {
        ...
        // 风扰动补偿参数
        nh_.param("/mission_aruco_land_node/wind_compensation/enable", wind_compensation_enable_, true);
        nh_.param("/mission_aruco_land_node/wind_compensation/position_filter_time_constant", position_filter_tc_, 0.2);
        nh_.param("/mission_aruco_land_node/wind_compensation/velocity_filter_time_constant", velocity_filter_tc_, 0.1);
        nh_.param("/mission_aruco_land_node/wind_compensation/wind_estimation_filter_tc", wind_estimation_filter_tc_, 1.0);
        nh_.param("/mission_aruco_land_node/wind_compensation/compensation_gain", wind_compensation_gain_, 0.3);
        nh_.param("/mission_aruco_land_node/wind_compensation/max_compensation", max_wind_compensation_, 0.2);
        nh_.param("/mission_aruco_land_node/wind_compensation/adaptive_gain_large", adaptive_gain_large_, 1.2);
        nh_.param("/mission_aruco_land_node/wind_compensation/adaptive_gain_medium", adaptive_gain_medium_, 1.0);
        nh_.param("/mission_aruco_land_node/wind_compensation/adaptive_gain_small", adaptive_gain_small_, 0.8);
        nh_.param("/mission_aruco_land_node/wind_compensation/large_error_threshold", large_error_threshold_, 0.3);
        nh_.param("/mission_aruco_land_node/wind_compensation/medium_error_threshold", medium_error_threshold_, 0.1);
        nh_.param("/mission_aruco_land_node/wind_compensation/position_prediction_time", position_prediction_time_, 0.2);
        nh_.param("/mission_aruco_land_node/wind_compensation/enable_debug", enable_wind_debug_, true);
        nh_.param("/mission_aruco_land_node/wind_compensation/debug_interval", wind_debug_interval_, 2.0);
        
        // 初始化滤波器
        position_filter_x_.set_Time_constant(position_filter_tc_);
        position_filter_y_.set_Time_constant(position_filter_tc_);
        velocity_filter_x_.set_Time_constant(velocity_filter_tc_);
        velocity_filter_y_.set_Time_constant(velocity_filter_tc_);
        wind_estimation_filter_x_.set_Time_constant(wind_estimation_filter_tc_);
        wind_estimation_filter_y_.set_Time_constant(wind_estimation_filter_tc_);
        ...
        nh_.param("/mission_aruco_land_node/lidar/fusion_weight", lidar_fusion_weight_, 1.0);
    ...
    // 新增：风扰动补偿控制输出
    std::pair<double, double> applyWindCompensation(double raw_output_x, double raw_output_y, double dt) {
        if (!wind_compensation_enable_) {
            return {raw_output_x, raw_output_y};
        }
        ...
        // 2. 风扰动估计和补偿
        updateWindEstimation(filtered_x, filtered_y, dt);
        
        // 3. 自适应增益调整
        double adaptive_gain_x = calculateAdaptiveGain(filtered_x, dt);
        double adaptive_gain_y = calculateAdaptiveGain(filtered_y, dt);
        
        // 4. 应用风补偿
        double compensated_x = filtered_x + wind_compensation_x_ * wind_compensation_gain_;
        double compensated_y = filtered_y + wind_compensation_y_ * wind_compensation_gain_;
        
        // 5. 自适应增益应用
        compensated_x *= adaptive_gain_x;
        compensated_y *= adaptive_gain_y;
        
        // 6. 最终输出滤波
        double final_x = velocity_filter_x_.apply(compensated_x, dt);
        double final_y = velocity_filter_y_.apply(compensated_y, dt);
        
        // 调试输出
        if (enable_wind_debug_) {
            static ros::Time last_wind_debug = ros::Time(0);
            if ((ros::Time::now() - last_wind_debug).toSec() > wind_debug_interval_) {
                ROS_INFO("Wind compensation: raw=[%.3f,%.3f], filtered=[%.3f,%.3f], wind_est=[%.3f,%.3f], final=[%.3f,%.3f]",
                         raw_output_x, raw_output_y, filtered_x, filtered_y, 
                         wind_compensation_x_, wind_compensation_y_, final_x, final_y);
                last_wind_debug = ros::Time::now();
            }
        }
    ...
    void updateWindEstimation(double control_x, double control_y, double dt) {
        // 记录控制输出历史
        control_history_x_.push_back(control_x);
        control_history_y_.push_back(control_y);
        
        // 保持历史记录长度
        const size_t max_history = 20;
        if (control_history_x_.size() > max_history) {
            control_history_x_.erase(control_history_x_.begin());
        }
        if (control_history_y_.size() > max_history) {
            control_history_y_.erase(control_history_y_.begin());
        }
        
        // 计算控制输出的趋势（风扰动估计）
        if (control_history_x_.size() >= 10) {
            double mean_x = 0.0, mean_y = 0.0;
            for (size_t i = control_history_x_.size() - 10; i < control_history_x_.size(); ++i) {
                mean_x += control_history_x_[i];
                mean_y += control_history_y_[i];
            }
            mean_x /= 10.0;
            mean_y /= 10.0;
            
            // 低通滤波风估计
            wind_compensation_x_ = wind_estimation_filter_x_.apply(mean_x, dt);
            wind_compensation_y_ = wind_estimation_filter_y_.apply(mean_y, dt);
            
            // 限制风补偿范围
            wind_compensation_x_ = std::clamp(wind_compensation_x_, -max_wind_compensation_, max_wind_compensation_);
            wind_compensation_y_ = std::clamp(wind_compensation_y_, -max_wind_compensation_, max_wind_compensation_);
        }
    }
    ...
    double calculateAdaptiveGain(double control_output, double dt) {
        // 基于控制输出幅度的自适应增益
        double output_magnitude = std::abs(control_output);
        
        if (output_magnitude > large_error_threshold_) {
            return adaptive_gain_large_;  // 大误差：增加增益
        } else if (output_magnitude > medium_error_threshold_) {
            return adaptive_gain_medium_; // 中误差：标准增益
        } else {
            return adaptive_gain_small_;  // 小误差：减少增益，提高稳定性
        }
    }
    ...
    std::pair<double, double> calculatePositionPrediction(double current_x, double current_y, double dt) {
        // 记录位置历史
        position_history_x_.push_back(current_x);
        position_history_y_.push_back(current_y);
        
        const size_t max_history = 5;
        if (position_history_x_.size() > max_history) {
            position_history_x_.erase(position_history_x_.begin());
        }
        if (position_history_y_.size() > max_history) {
            position_history_y_.erase(position_history_y_.begin());
        }
        
        // 计算位置变化趋势
        if (position_history_x_.size() >= 3) {
            double vel_x = (position_history_x_.back() - position_history_x_[position_history_x_.size()-3]) / (2.0 * dt);
            double vel_y = (position_history_y_.back() - position_history_y_[position_history_y_.size()-3]) / (2.0 * dt);
            
            // 预测未来位置（前馈补偿）
            double prediction_time = position_prediction_time_;
            double predicted_x = current_x + vel_x * prediction_time;
            double predicted_y = current_y + vel_y * prediction_time;
    ...
    void executeRoughLandMode(double dt) {
        ...
        double raw_output_x = pid_x_.compute(error_x, dt);
        double raw_output_y = pid_y_.compute(error_y, dt);
        
        // 应用风扰动补偿
        auto compensated_output = applyWindCompensation(raw_output_x, raw_output_y, dt);
        double output_x = compensated_output.first;
        double output_y = compensated_output.second;
        ...
        // 计算偏航角速率并发布所有控制量（目标朝向为东方）
        float target_yaw = target_east_yaw_;  // 使用配置的东向目标朝向
        float yaw_rate = calculateYawRate(target_yaw, dt);
        send_velocity_setpoint(output_x, output_y, output_z, yaw_rate);
    ...
    void executeFineLandMode(double dt) {
        ...
        // 计算偏航角速率并发布MAVROS速度控制命令（目标朝向为东方）
        float target_yaw = target_east_yaw_;  // 使用配置的东向目标朝向
        float yaw_rate = calculateYawRate(target_yaw, dt);
        send_velocity_setpoint(output_x, output_y, output_z, yaw_rate);
    ...
    void executeCornerGuidedLandMode(double dt) {
        ...
        float target_yaw = target_east_yaw_;  // 东向为0度（可配置）
        if (platform_yaw_valid_) {
            // 基于四角标记的平台朝向，调整飞机朝向使其朝向东方
            // 不再使用 -platform_yaw_，而是直接使用配置的东向目标
            target_yaw = target_east_yaw_;
        }
        float yaw_rate = calculateYawRate(target_yaw, dt);
    ...
    float calculateYawRate(float target_yaw, double dt) {
        ...
        if (enable_yaw_debug_ && (ros::Time::now() - last_yaw_debug_print).toSec() > yaw_debug_print_interval_) {
            ROS_INFO("Yaw control: current=%.1f°, target=%.1f°, error=%.1f°, rate=%.2f rad/s", 
                     current_yaw_ * 180.0 / M_PI, target_yaw * 180.0 / M_PI, 
                     yaw_error * 180.0 / M_PI, yaw_rate);
    ...
    // 风扰动补偿相关参数
    bool wind_compensation_enable_;      // 启用风扰动补偿
    LowPassFilter position_filter_x_;    // X轴位置滤波器
    LowPassFilter position_filter_y_;    // Y轴位置滤波器
    LowPassFilter velocity_filter_x_;    // X轴速度滤波器
    LowPassFilter velocity_filter_y_;    // Y轴速度滤波器
    LowPassFilter wind_estimation_filter_x_; // X轴风估计滤波器
    LowPassFilter wind_estimation_filter_y_; // Y轴风估计滤波器
    
    // 风补偿参数
    double wind_compensation_x_;         // X轴风补偿值
    double wind_compensation_y_;         // Y轴风补偿值
    double wind_compensation_gain_;      // 风补偿增益
    double max_wind_compensation_;       // 最大风补偿限制
    
    // 自适应增益参数
    double adaptive_gain_large_;         // 大误差时的增益
    double adaptive_gain_medium_;        // 中误差时的增益
    double adaptive_gain_small_;         // 小误差时的增益
    double large_error_threshold_;       // 大误差阈值
    double medium_error_threshold_;      // 中误差阈值
    
    // 位置预测参数
    double position_prediction_time_;    // 位置预测时间
    std::vector<double> control_history_x_; // 控制输出历史（用于风估计）
    std::vector<double> control_history_y_;
    std::vector<double> position_history_x_; // 位置历史（用于预测）
    std::vector<double> position_history_y_;
    
    // 滤波器时间常数
    double position_filter_tc_;          // 位置滤波器时间常数
    double velocity_filter_tc_;          // 速度滤波器时间常数
    double wind_estimation_filter_tc_;   // 风估计滤波器时间常数
    
    // 调试参数
    bool enable_wind_debug_;             // 启用风扰动调试
    double wind_debug_interval_;         // 风扰动调试输出间隔
