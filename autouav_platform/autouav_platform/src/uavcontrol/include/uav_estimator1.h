...
class UAV_estimator
{
    public:
        ...
        // 新增：飞控与任务状态发布
        ros::Publisher px4_armed_pub;               // /<uav_name>/prometheus/status/armed
        ros::Publisher takeoff_completed_pub;       // /<uav_name>/prometheus/status/takeoff_completed
        ros::Publisher landing_completed_pub;       // /<uav_name>/prometheus/status/landing_completed
        ros::Publisher gps_navsatfix_pub;           // /<uav_name>/prometheus/status/navsat
        // 定时器
        ros::Timer timer_px4_vision_pub;
        ros::Timer timer_uav_state_pub;
        ros::Timer timer_rviz_pub;
        ros::Timer ground_station_info_timer;
        ros::Timer timer_flight_status_pub; // 新增：飞行状态发布定时器
