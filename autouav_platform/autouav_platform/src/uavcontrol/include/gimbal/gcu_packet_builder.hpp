...
public:
  ...
  std::array<uint8_t, PACKET_LENGTH> buildEmptyCommandPacket() const {
    std::array<uint8_t, PACKET_LENGTH> pkt{};
    ...
    // 11 flags: set control invalid (0x00)
    pkt[11] = 0x00;
  ...
  std::array<uint8_t, PACKET_LENGTH> buildPipModePacket(uint8_t pip_mode) const {
    std::array<uint8_t, PACKET_LENGTH> pkt{};
    ...
    // 11 flags: set control invalid (0x00) for pip mode command
    pkt[11] = 0x00;
    ...
    // PIP mode parameter (TT) - clamp to valid range [0,4]
    uint8_t clamped_mode = (pip_mode > 4) ? 4 : pip_mode;
    
    // Set pip mode parameter at a suitable position (using byte 5 for parameter storage)
    pkt[5] = clamped_mode;

    // 69 command: pip mode (0x74)
    pkt[69] = 0x74;
