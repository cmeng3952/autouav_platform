...
int main(int argc, char** argv) {
  ...
  std::string local_ip = "192.168.144.101";
  int local_port = 2338;
  ...
  ROS_INFO("Gimbal control node started. Keyboard optional: WASD pitch/yaw, QE roll, R center, V point down, P cycle PIP, 1-4 PIP modes, Enter toggle smooth, +/- step size, 0 reset. Press ESC to exit.");
  ...
  while (ros::ok()) {
    ...
    if (k != -1) {
      ...
      else if (k == 'v' || k == 'V') controller->pointDown();
      else if (k == 'p' || k == 'P') controller->cyclePipMode();
      else if (k >= '1' && k <= '4') controller->setPipMode(static_cast<uint8_t>(k - '0'));
