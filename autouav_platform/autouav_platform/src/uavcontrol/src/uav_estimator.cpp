...
UAV_estimator::UAV_estimator(ros::NodeHandle &nh): nh(nh)
{
    ...
    nh.param<int>("control/location_source", location_source, uavcontrol_msgs::UAVState::GPS);
    // 【参数】最大安全速度
    ...
    else if (location_source == uavcontrol_msgs::UAVState::GPS || location_source == uavcontrol_msgs::UAVState::RTK)
    {
        ...
        set_local_pose_offset_sub = nh.subscribe<uavcontrol_msgs::GPSData>(uav_name + "/prometheus/set_local_offset_pose", 1, &UAV_estimator::set_local_pose_offset_cb, this);
        // 【发布】ENU坐标系下的位置偏移量
        local_pose_offset_pub = nh.advertise<uavcontrol_msgs::OffsetPose>(uav_name + "/prometheus/offset_pose", 1);
        // 【订阅】GPS卫星数量
    ...
    else if (location_source == uavcontrol_msgs::UAVState::UWB)
    {
        // 【订阅】UWB
        uwb_sub = nh.subscribe<uavcontrol_msgs::LinktrackNodeframe2>("/nlink_linktrack_nodeframe2", 10, &UAV_estimator::uwb_cb, this);
    }
    else if (location_source == uavcontrol_msgs::UAVState::BSA_SLAM)
    {
        // 【订阅】BSA_SLAM估计位置
    ...
    {
        text_info.MessageType = uavcontrol_msgs::TextInfo::WARN;
    ...
    // 【订阅】地面站修改ROS参数
    ros_param_set_sub = nh.subscribe<uavcontrol_msgs::ParamSettings>(uav_name + "/prometheus/param_settings", 1, &UAV_estimator::param_set_cb, this);

    // 【订阅】桨叶收纳完成状态
    prop_folded_sub = nh.subscribe<std_msgs::Bool>(uav_name + "/uavcontrol/prop_folded_state", 1, &UAV_estimator::prop_folded_cb, this);

    // 【发布】无人机状态合集,包括位置\速度\姿态\模式等,供上层节点使用
    uav_state_pub = nh.advertise<uavcontrol_msgs::UAVState>(uav_name + "/prometheus/state", 1);
...
// 【发布】相机(d435i)里程计，用于需要相机坐标的数据消费者
    ...
    // 【发布】运行状态信息(-> 通信节点 -> 地面站)
    ground_station_info_pub = nh.advertise<uavcontrol_msgs::TextInfo>("/uav" + std::to_string(uav_id) + "/prometheus/text_info", 1);

    //
    gps_navsatfix_pub = nh.advertise<sensor_msgs::NavSatFix>(uav_name + "/prometheus/status/navsat", 1, true);

    if (location_source == uavcontrol_msgs::UAVState::MOCAP || location_source == uavcontrol_msgs::UAVState::BSA_SLAM || location_source == uavcontrol_msgs::UAVState::T265 || location_source == uavcontrol_msgs::UAVState::viobot || location_source == uavcontrol_msgs::UAVState::GAZEBO || location_source == uavcontrol_msgs::UAVState::UWB|| location_source == uavcontrol_msgs::UAVState::VINS || location_source == uavcontrol_msgs::UAVState::MID360)
    {
    ...
    timer_uav_state_pub = nh.createTimer(ros::Duration(1.0), &UAV_estimator::timercb_pub_uav_state, this);
    ...
    // 【新增定时器】定时发布飞行状态（arming、GPS、起飞/降落完成）
    timer_flight_status_pub = nh.createTimer(ros::Duration(1.0), &UAV_estimator::timercb_pub_flight_status, this);
    ...
    uav_state.gps_status = uavcontrol_msgs::UAVState::GPS_FIX_TYPE_NO_GPS;
    uav_state.gps_num = 0;
    uav_state.position[0] = 0.0;
    uav_state.position[1] = 0.0;
    uav_state.position[2] = 0.0;
    //该经纬度为阿木实验室测试场地(小花园)的经纬度
    uav_state.latitude = 35.4004440;
    uav_state.longitude = 116.4398094;
...
{
    if (location_source == uavcontrol_msgs::UAVState::GAZEBO)
    {
        vision_pose = gazebo_pose;
    }
    else if (location_source == uavcontrol_msgs::UAVState::MOCAP)
    {
...
void UAV_estimator::timercb_rviz(const ros::TimerEvent &e)
{
    ...
    if(location_source == uavcontrol_msgs::UAVState::T265)
    {
        //  |----坐标系相对信息设置  偏移量  无人机相对于世界坐标系的坐标
...
void UAV_estimator::px4_global_rel_alt_cb(const std_msgs::Float64::ConstPtr &msg)
{
   ...
   // ROS_INFO(" rel_alt: %.5f", uav_state.rel_alt);
}

// void UAV_estimator::timercb_pub_flight_status(const ros::TimerEvent &e)
// {
//     // Armed 状态
//     // std_msgs::Bool armed_msg;
//     // armed_msg.data = uav_state.armed;
//     // px4_armed_pub.publish(armed_msg);

//     // // GPS NavSatFix - 使用最近一次从 MAVROS 接收的 global_position/global
//     // sensor_msgs::NavSatFix navsat = sensor_msgs::NavSatFix();
//     // navsat.header.stamp = ros::Time::now();
//     // navsat.status.status = sen
//     // navsat.latitude = uav_state.latitude;
//     // navsat.longitude = uav_state.longitude;
//     // navsat.altitude = uav_state.altitude;
//     // gps_navsatfix_pub.publish(navsat);

//     // // 任务完成状态：降落完成 = 上锁 且 相对高度 < 1m
//     // std_msgs::Bool landing_completed;
//     // landing_completed.data = (!uav_state.armed) && (uav_state.rel_alt < 1.0);
//     // landing_completed_pub.publish(landing_completed);

//     // // 任务完成状态：起飞完成 = 解锁 且 相对高度 > 5m
//     // std_msgs::Bool takeoff_completed;
//     // takeoff_completed.data = (uav_state.armed) && (uav_state.rel_alt > 5.0);
//     // takeoff_completed_pub.publish(takeoff_completed);
// }


// 辅助函数：计算高度变化率（简单一阶差分）
double UAV_estimator::calc_height_rate() {
    static double last_alt = 0.0;
    static ros::Time last_time = ros::Time::now();
    
    double current_alt = uav_state.rel_alt;
    ros::Time current_time = ros::Time::now();
    double dt = (current_time - last_time).toSec();
    
    if (dt < 0.01) {  // 避免时间间隔过小导致误差
        return 0.0;
    }
    
    double rate = (current_alt - last_alt) / dt;
...
bool UAV_estimator::check_valid_data() {
    bool is_valid = check_uav_odom() == 9;
    if (!is_valid) {
        ROS_WARN("[%s] 数据无效，进入DATA_ERR", node_name.c_str());
    }
    return is_valid;
}
...
void UAV_estimator::timercb_pub_flight_status(const ros::TimerEvent &e) {
    ...
    double height_rate = calc_height_rate();  // 计算高度变化率（上升为正，下降为负）

    // 超时重置标志
    if (takeoff_completed_ && (current_time - takeoff_trigger_time_.toSec() > STATE_HOLD_DURATION)) {
        takeoff_completed_ = false;
        takeoff_stable_count_ = 0;
        ROS_INFO("[%s] 起飞完成标志超时重置", node_name.c_str());
    }
    if (landing_completed_ && (current_time - landing_trigger_time_.toSec() > STATE_HOLD_DURATION)) {
        landing_completed_ = false;
        landing_stable_count_ = 0;
        ROS_INFO("[%s] 降落完成标志超时重置", node_name.c_str());
    }

    FlightPhase new_phase = current_phase_;
    bool has_valid_data = check_valid_data();

    // 1. 数据异常处理
    if (!has_valid_data) {
        if (current_phase_ != FlightPhase::DATA_ERR) {
            new_phase = FlightPhase::DATA_ERR;
            ROS_WARN("[%s] 数据异常，切换到DATA_ERR（高度: %.2f, 解锁: %s）",
                     node_name.c_str(), uav_state.rel_alt, uav_state.armed ? "是" : "否");
        }
    } else {
        // 2. 数据恢复处理（从DATA_ERR退出）
        if (current_phase_ == FlightPhase::DATA_ERR) {
            // 恢复逻辑：结合解锁状态、高度和模式
            if (uav_state.armed) {
                if (uav_state.rel_alt > TAKEOFF_HEIGHT_THRESHOLD) {
                    new_phase = FlightPhase::FLYING;
                } else if (uav_state.rel_alt > takeoff_base_height_ + HEIGHT_CHANGE_THRESHOLD) {
                    new_phase = FlightPhase::TAKING_OFF;
                } else {
                    new_phase = FlightPhase::READY;  // 解锁但未起飞
                }
            } else {
                if (uav_state.rel_alt < LANDING_HEIGHT_THRESHOLD) {
                    new_phase = FlightPhase::READY;
                } else {
                    new_phase = FlightPhase::LANDING;
                }
            }
            ROS_INFO("[%s] 数据恢复，切换到: %s（高度: %.2f, 速率: %.2f）",
                     node_name.c_str(), get_phase_str(new_phase).c_str(),
                     uav_state.rel_alt, height_rate);
        } 
        // 3. 正常状态流转
        else {
            switch (current_phase_) {
                case FlightPhase::READY:
                    ...
                    if (uav_state.armed) {
                        // 记录起飞基准高度（首次解锁时的高度）
                        if (takeoff_base_height_ < 0.1) {
                            takeoff_base_height_ = uav_state.rel_alt;
                            ROS_INFO("[%s] 记录起飞基准高度: %.2f米",
                                     node_name.c_str(), takeoff_base_height_);
                        }
                        // 高度超过基准+0.5m，且上升速率>0.1m/s -> 确认起飞启动
                        if (uav_state.rel_alt > takeoff_base_height_ + 0.5 && height_rate > HEIGHT_CHANGE_THRESHOLD) {
                            new_phase = FlightPhase::TAKING_OFF;
                            is_takeoff_started_ = true;
                            ROS_INFO("[%s] 起飞启动（当前高度: %.2f, 上升速率: %.2f）",
                                     node_name.c_str(), uav_state.rel_alt, height_rate);
                        }
                    }
                    break;
                ...
                case FlightPhase::TAKING_OFF:
                    // 已解锁且未完成起飞
                    if (uav_state.armed && is_takeoff_started_) {
                        // 高度>5m，且连续5个周期稳定（速率<0.3m/s）-> 起飞完成
                        if (uav_state.rel_alt > TAKEOFF_HEIGHT_THRESHOLD) {
                            if (fabs(height_rate) < 0.3) {  // 高度趋于稳定
                                takeoff_stable_count_++;
                                if (takeoff_stable_count_ >= STABLE_COUNT_THRESHOLD) {
                                    new_phase = FlightPhase::FLYING;
                                    if (!takeoff_completed_) {
                                        takeoff_completed_ = true;
                                        takeoff_trigger_time_ = ros::Time::now();
                                        ROS_INFO("[%s] 起飞完成（高度: %.2f米，稳定计数: %d）",
                                                 node_name.c_str(), uav_state.rel_alt, takeoff_stable_count_);
                                    }
                                }
                            } else {
                                takeoff_stable_count_ = 0;  // 高度仍在快速变化，重置计数
                            }
                        }
                    } 
                    // 起飞中意外上锁 -> 进入降落
                        ...
                        new_phase = FlightPhase::LANDING;
                        is_landing_started_ = true;
                        ROS_INFO("[%s] 起飞中上锁，进入降落（高度: %.2f）",
                                 node_name.c_str(), uav_state.rel_alt);
                    }
                    break;

                case FlightPhase::FLYING:
                    // 解锁状态下，满足以下任一条件 -> 进入降落
                    // 1. 收到降落模式（LAND）；2. 上锁；3. 高度持续下降（速率<-0.1m/s）
                    bool is_landing_condition = (uav_state.mode == "LAND") 
                                             || (!uav_state.armed) 
                                             || (height_rate < -HEIGHT_CHANGE_THRESHOLD);
                    
                    if (is_landing_condition) {
                        new_phase = FlightPhase::LANDING;
                        is_landing_started_ = true;
                        ROS_INFO("[%s] 进入降落阶段（模式: %s, 速率: %.2f, 解锁: %s）",
                                 node_name.c_str(), uav_state.mode.c_str(),
                                 height_rate, uav_state.armed ? "是" : "否");
                    }
                    break;

                case FlightPhase::LANDING:
                    // 降落中状态判断
                    if (is_landing_started_) {
                        // 上锁状态下，高度<1m且速率接近0（稳定着陆）-> 降落完成
                        if (!uav_state.armed) {
                            if (uav_state.rel_alt < LANDING_HEIGHT_THRESHOLD) {
                                if (fabs(height_rate) < 0.1) {  // 高度几乎不变（已着陆）
                                    landing_stable_count_++;
                                    if (landing_stable_count_ >= STABLE_COUNT_THRESHOLD) {
                                        new_phase = FlightPhase::READY;
                                        if (!landing_completed_) {
                                            landing_completed_ = true;
                                            landing_trigger_time_ = ros::Time::now();
                                            ROS_INFO("[%s] 降落完成（高度: %.2f米，稳定计数: %d）",
                                                     node_name.c_str(), uav_state.rel_alt, landing_stable_count_);
                                        }
                                    }
                                } else {
                                    landing_stable_count_ = 0;  // 仍在下降，重置计数
                                }
                            }
                        } 
                        // 降落中意外解锁 -> 回到飞行
                        else if (uav_state.armed && height_rate > HEIGHT_CHANGE_THRESHOLD) {
                            new_phase = FlightPhase::FLYING;
                            ROS_INFO("[%s] 降落中解锁并上升，回到飞行阶段", node_name.c_str());
                        }
                    }
                    break;
    ...
    // 更新飞行阶段字符串（辅助函数get_phase_str见下方）
    uav_state.flight_phase = get_phase_str(new_phase);

    // 强制发布最新状态
    uav_state.takeoff_completed = takeoff_completed_;
    uav_state.landing_completed = landing_completed_;
    uav_state.header.stamp = ros::Time::now();
    uav_state_pub.publish(uav_state);
}

// 辅助函数：将FlightPhase转换为字符串（.cpp中实现）
std::string UAV_estimator::get_phase_str(FlightPhase phase) {
    switch (phase) {
...
// 桨叶状态回调函数
void UAV_estimator::prop_folded_cb(const std_msgs::Bool::ConstPtr& msg) {
    std::lock_guard<std::mutex> lock(prop_mutex);  // 线程安全
    uav_state.prop_folded_state = msg->data;  // 存储接收到的状态
    uav_state_pub.publish(uav_state);
...
void UAV_estimator::px4_att_cb(const sensor_msgs::Imu::ConstPtr &msg)
{
    ...
    uav_state.attitude[0] = euler_fcu[0];  // roll（弧度）
    uav_state.attitude[1] = euler_fcu[1];  // pitch（弧度）
    uav_state.attitude[2] = euler_fcu[2];  // yaw（弧度）

    uav_state.attitude_rpy[0] = uav_state.attitude[0] * rad_to_deg;  // roll
    uav_state.attitude_rpy[1] = uav_state.attitude[1] * rad_to_deg;  //pitch
    uav_state.attitude_rpy[2] = uav_state.attitude[2] * rad_to_deg;  //yaw
...
void UAV_estimator::uwb_cb(const uavcontrol_msgs::LinktrackNodeframe2::ConstPtr &msg)
{
...
void UAV_estimator::check_uav_state()
{
    ...
    {
        text_info.MessageType = uavcontrol_msgs::TextInfo::ERROR;
    ...
    {
        if(location_source == uavcontrol_msgs::UAVState::GPS)
        {
            text_info.MessageType = uavcontrol_msgs::TextInfo::WARN;
        ...
        if(location_source == uavcontrol_msgs::UAVState::RTK)
        {
            text_info.MessageType = uavcontrol_msgs::TextInfo::WARN;
...
int UAV_estimator::check_uav_odom()
{
    ...
    if (location_source == uavcontrol_msgs::UAVState::GAZEBO && (time_now - get_gazebo_stamp).toSec() > GAZEBO_TIMEOUT)
    {
        return 1;
    }
    else if (location_source == uavcontrol_msgs::UAVState::MOCAP && (time_now - get_mocap_stamp).toSec() > MOCAP_TIMEOUT)
    {
        return 1;
    }
    else if (location_source == uavcontrol_msgs::UAVState::T265 && (time_now - get_t265_stamp).toSec() > T265_TIMEOUT)
    {
        return 1;
    }
    else if (location_source == uavcontrol_msgs::UAVState::viobot && (time_now - get_viobot_stamp).toSec() > VIOBOT_TIMEOUT)
    {
        return 1;
    }
    else if (location_source == uavcontrol_msgs::UAVState::MID360 && (time_now - get_mid360_stamp).toSec() > MID360_TIMEOUT)
    {
        return 1;
    }
    // UWB
    else if (location_source == uavcontrol_msgs::UAVState::UWB && (time_now - get_uwb_stamp).toSec() > UWB_TIMEOUT)
    {
        return 1;
    }
    else if (location_source == uavcontrol_msgs::UAVState::VINS && (time_now - get_vins_stamp).toSec() > VINS_TIMEOUT)
    {
        return 1;
    }
    else if (location_source == uavcontrol_msgs::UAVState::BSA_SLAM && (time_now - get_BSA_SLAM_stamp).toSec() > BSA_SLAM_TIMEOUT)
    {
        return 1;
    }
    else if ((location_source == uavcontrol_msgs::UAVState::GPS || location_source == uavcontrol_msgs::UAVState::RTK) && (time_now - get_gps_stamp).toSec() > GPS_TIMEOUT)
    {
        return 7;
    }
    ...
    if (location_source == uavcontrol_msgs::UAVState::GPS)
    {
        if (uav_state.gps_status < uavcontrol_msgs::UAVState::GPS_FIX_TYPE_3D_FIX)
        {
            return 4;
        }
        else if(uav_state.gps_status > uavcontrol_msgs::UAVState::GPS_FIX_TYPE_3D_FIX)
        {
            return 5;
        }
    }
    ...
    if (location_source == uavcontrol_msgs::UAVState::RTK)
    {
        ...
        else if (uav_state.gps_status <= uavcontrol_msgs::UAVState::GPS_FIX_TYPE_RTK_FLOATR)
        {
            return 5;
        }
    }
...
void UAV_estimator::printf_uav_state()
{
    ...
    {
    case uavcontrol_msgs::UAVState::MOCAP:
        ...
        break;
    case uavcontrol_msgs::UAVState::T265:
        ...
        break;
    case uavcontrol_msgs::UAVState::OPTICAL_FLOW:
        ...
        break;
    case uavcontrol_msgs::UAVState::viobot:
        ...
        break;
    case uavcontrol_msgs::UAVState::MID360:
        ...
        break;
    case uavcontrol_msgs::UAVState::GAZEBO:
        ...
        break;
    case uavcontrol_msgs::UAVState::FAKE_ODOM:
        cout << GREEN << "Location: [ FAKE_ODOM ] " << TAIL << endl;
        break;
    case uavcontrol_msgs::UAVState::GPS:
        cout << GREEN << "Location: [ GPS ] " << TAIL;
        printf_gps_status();
        break;
    case uavcontrol_msgs::UAVState::RTK:
        cout << GREEN << "Location: [ RTK ] " << TAIL;
        printf_gps_status();
        break;
    case uavcontrol_msgs::UAVState::UWB:
        ...
        break;
    case uavcontrol_msgs::UAVState::VINS:
        ...
        break;
    case uavcontrol_msgs::UAVState::BSA_SLAM:
...
void UAV_estimator::printf_gps_status()
{
    ...
    if (location_source == uavcontrol_msgs::UAVState::GPS)
    {
        ...
        {
        case uavcontrol_msgs::UAVState::GPS_FIX_TYPE_NO_GPS:
            cout << RED << " [GPS_FIX_TYPE_NO_GPS] " << TAIL << endl;
            break;
        case uavcontrol_msgs::UAVState::GPS_FIX_TYPE_NO_FIX:
            cout << RED << " [GPS_FIX_TYPE_NO_FIX] " << TAIL << endl;
            break;
        case uavcontrol_msgs::UAVState::GPS_FIX_TYPE_2D_FIX:
            cout << YELLOW << " [GPS_FIX_TYPE_2D_FIX] " << TAIL << endl;
            break;
        case uavcontrol_msgs::UAVState::GPS_FIX_TYPE_3D_FIX:
    ...
    if (location_source == uavcontrol_msgs::UAVState::RTK)
    {
        ...
        case uavcontrol_msgs::UAVState::GPS_FIX_TYPE_3D_FIX:
            ...
            break;
        case uavcontrol_msgs::UAVState::GPS_FIX_TYPE_DGPS:
            cout << YELLOW << " [GPS_FIX_TYPE_DGPS] " << TAIL << endl;
            break;
        case uavcontrol_msgs::UAVState::GPS_FIX_TYPE_RTK_FLOATR:
            cout << YELLOW << " [GPS_FIX_TYPE_RTK_FLOATR] " << TAIL << endl;
            break;
        case uavcontrol_msgs::UAVState::GPS_FIX_TYPE_RTK_FIXEDR:
...
void UAV_estimator::printf_param()
{
    ...
    else if (location_source == uavcontrol_msgs::UAVState::GPS)
    {
        cout << GREEN << "location_source: [GPS] " << TAIL << endl;
    }
    else if (location_source == uavcontrol_msgs::UAVState::RTK)
    {
...
void UAV_estimator::set_local_pose_offset_cb(const uavcontrol_msgs::GPSData::ConstPtr &msg)
{
...
void UAV_estimator::param_set_cb(const uavcontrol_msgs::ParamSettings::ConstPtr &msg)
{
    try
    {
