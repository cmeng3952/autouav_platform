...
UAV_controller::UAV_controller(ros::NodeHandle &nh) : nh(nh)
{
    ...
    // 【参数】控制器标志位,0具体说明见CONTOLLER_FLAG说明
    ...
    geo_fence_sub = nh.subscribe<uavcontrol_msgs::GeoFence>("/uav" + std::to_string(uav_id) + "/prometheus/geo_fence", 
                                                            2, &UAV_controller::geo_fence_cb, this);

    // 【订阅】无人机控制指令(用于COMMAND_CONTROL模式)
    uav_cmd_sub =
        nh.subscribe<uavcontrol_msgs::UAVCommand>("/uav" + std::to_string(uav_id) + "/prometheus/command",
                                                  2,
                                                  &UAV_controller::uav_cmd_cb, this);

    // 【订阅】无人机状态信息
    uav_state_sub = nh.subscribe<uavcontrol_msgs::UAVState>("/uav" + std::to_string(uav_id) + "/prometheus/state",
                                                            2,
                                                            &UAV_controller::uav_state_cb, this);

    // 【订阅】无人机设置指令
    uav_setup_sub =
        nh.subscribe<uavcontrol_msgs::UAVSetup>("/uav" + std::to_string(uav_id) + "/prometheus/setup",
                                                2,
    ...
    offset_pose_sub =
        nh.subscribe<uavcontrol_msgs::OffsetPose>("/uav" + std::to_string(uav_id) + "/prometheus/offset_pose",
                                                  2,
                                                  &UAV_controller::offset_pose_cb, this);

    // 【订阅】地面站修改ROS参数
    ros_param_set_sub = 
        nh.subscribe<uavcontrol_msgs::ParamSettings>("/uav" + std::to_string(uav_id) + "/prometheus/param_settings",
                                                  2,
    ...
    uav_control_state_pub =
        nh.advertise<uavcontrol_msgs::UAVControlState>("/uav" + std::to_string(uav_id) + "/prometheus/control_state", 1);
...
void UAV_controller::mainloop()
{
    if (control_state == CONTROL_STATE::RC_POS_CONTROL || control_state == CONTROL_STATE::COMMAND_CONTROL)
    {
        ...
        {
            // 与PX4断开连接，直接返回
            this->text_info.MessageType = uavcontrol_msgs::TextInfo::ERROR;
    ...
    case CONTROL_STATE::LAND_CONTROL:
        ...
        if (uav_state.location_source == uavcontrol_msgs::UAVState::GPS || uav_state.location_source == uavcontrol_msgs::UAVState::RTK)
        {
...
void UAV_controller::set_command_des()
{
    ...
    else if (uav_command.Agent_CMD == uavcontrol_msgs::UAVCommand::Current_Pos_Hover)
    {
        // 【Current_Pos_Hover】 当前位置悬停
        if (uav_command_last.Agent_CMD != uavcontrol_msgs::UAVCommand::Current_Pos_Hover)
        {
            Hover_position[0] = uav_pos[0] - offset_pose.x;
            Hover_position[1] = uav_pos[1] - offset_pose.y;
            Hover_position[2] = uav_pos[2];
    ...
    else if (uav_command.Agent_CMD == uavcontrol_msgs::UAVCommand::Land)
    {
        // 【Land】 降落，直接使用LAND_CONTROL
        control_state = CONTROL_STATE::LAND_CONTROL;
    }
    
    else if (uav_command.Agent_CMD == uavcontrol_msgs::UAVCommand::RTL)
        {
        // 【RTL】 返航模式
    ...
    else if (uav_command.Agent_CMD == uavcontrol_msgs::UAVCommand::Move)
    {
        // 【Move】 移动，移动子模式的区别详见UAVCommand.msg中的说明
        if (uav_command.Move_mode == uavcontrol_msgs::UAVCommand::XYZ_POS)
        {
            // 【XYZ_POS】XYZ惯性系定点控制
        ...
        else if (uav_command.Move_mode == uavcontrol_msgs::UAVCommand::XY_VEL_Z_POS)
        {
            // 【XYZ_POS】Z轴定高，XY速度控制
            pos_des[0] = 0.0;
            pos_des[1] = 0.0;
        ...
        else if (uav_command.Move_mode == uavcontrol_msgs::UAVCommand::XYZ_VEL)
        {
            pos_des << 0.0, 0.0, 0.0;
            vel_des[0] = uav_command.velocity_ref[0];
            vel_des[1] = uav_command.velocity_ref[1];
        ...
        else if (uav_command.Move_mode == uavcontrol_msgs::UAVCommand::XYZ_POS_BODY)
        {
        ...
        else if (uav_command.Move_mode == uavcontrol_msgs::UAVCommand::TRAJECTORY)
        {
        ...
        else if (uav_command.Move_mode == uavcontrol_msgs::UAVCommand::XYZ_ATT)
        {
        ...
        else if (uav_command.Move_mode == uavcontrol_msgs::UAVCommand::LAT_LON_ALT)
        {
...
void UAV_controller::uav_cmd_cb(const uavcontrol_msgs::UAVCommand::ConstPtr &msg)
{
    ...
    // 绝对悬停指令触发
    if (msg->Control_Level == uavcontrol_msgs::UAVCommand::ABSOLUTE_CONTROL &&
        msg->Agent_CMD == uavcontrol_msgs::UAVCommand::Current_Pos_Hover)
    {
    ...
    if (uav_command.Control_Level == uavcontrol_msgs::UAVCommand::ABSOLUTE_CONTROL &&
        msg->Control_Level != uavcontrol_msgs::UAVCommand::ABSOLUTE_CONTROL)
    {
        // 如果不是解除绝对控制，其他指令不响应
        if (msg->Control_Level != uavcontrol_msgs::UAVCommand::EXIT_ABSOLUTE_CONTROL)
        {
...
void UAV_controller::send_pos_cmd_to_px4_original_controller()
{
    ...
    if (control_state == CONTROL_STATE::COMMAND_CONTROL)
    {
            ...
            uav_command.Agent_CMD == uavcontrol_msgs::UAVCommand::Current_Pos_Hover)
        {
            ...
            vel_control = false;
        ...
        else if (uav_command.Agent_CMD == uavcontrol_msgs::UAVCommand::RTL)
                {
                    ...
                    // 使用类成员变量 px4_set_mode_client
                    if (px4_set_mode_client.call(rtl_set_mode) && rtl_set_mode.response.mode_sent) {
        ...
        {
            if (uav_command.Move_mode == uavcontrol_msgs::UAVCommand::XYZ_POS ||
                uav_command.Move_mode == uavcontrol_msgs::UAVCommand::XYZ_POS_BODY)
            {
                send_pos_setpoint(pos_des, yaw_des);
                vel_control = false;
            }
                     ...
                     uav_command.Move_mode == uavcontrol_msgs::UAVCommand::XYZ_VEL_BODY)
            {
                ...
                else
                {
                    send_vel_setpoint(vel_des, yaw_des);
                     ...
                     uav_command.Move_mode == uavcontrol_msgs::UAVCommand::XY_VEL_Z_POS_BODY)
            {
                if (uav_command.Yaw_Rate_Mode)
                {
...
void UAV_controller::uav_state_cb(const uavcontrol_msgs::UAVState::ConstPtr &msg)
{
    uav_state = *msg;
...
int UAV_controller::check_failsafe()
{
    ...
    if ((ros::Time::now() - rc_input.rcv_stamp).toSec() > 5.0 /* 秒 yuan 1.5 */)
    {
...
void UAV_controller::uav_setup_cb(const uavcontrol_msgs::UAVSetup::ConstPtr &msg)
{
    if (msg->cmd == uavcontrol_msgs::UAVSetup::ARMING)
    {
        arm_disarm_func(msg->arming);
    }
    else if (msg->cmd == uavcontrol_msgs::UAVSetup::SET_PX4_MODE)
    {
        set_px4_mode_func(msg->px4_mode);
    }
    else if (msg->cmd == uavcontrol_msgs::UAVSetup::REBOOT_PX4)
    {
        reboot_PX4();
    }
    else if (msg->cmd == uavcontrol_msgs::UAVSetup::SET_CONTROL_MODE)
    {
        // todo more test
...
void UAV_controller::printf_control_state()
{
    ...
    if (control_state == CONTROL_STATE::COMMAND_CONTROL)
    {
        ...
        {
        case uavcontrol_msgs::UAVCommand::Init_Pos_Hover:
        ...
        case uavcontrol_msgs::UAVCommand::Current_Pos_Hover:
            cout << GREEN << "Command: [ Current_Pos_Hover ] " << TAIL << endl;
            cout << GREEN << "Hover_Pos [X Y Z] : " << Hover_position[0] << " [ m ] " << Hover_position[1] << " [ m ] " << Hover_position[2] << " [ m ] " << TAIL << endl;
            break;

        case uavcontrol_msgs::UAVCommand::Land:
            cout << GREEN << "Command: [ Land ] " << TAIL << endl;
            break;

        case uavcontrol_msgs::UAVCommand::Move:
...
void UAV_controller::offset_pose_cb(const uavcontrol_msgs::OffsetPose::ConstPtr &msg)
{
    if (uav_state.location_source == uavcontrol_msgs::UAVState::GPS || uav_state.location_source == uavcontrol_msgs::UAVState::RTK)
    {
        offset_pose = *msg;
...
void UAV_controller::param_set_cb(const uavcontrol_msgs::ParamSettings::ConstPtr &msg)
{
    try
    {
...
void UAV_controller::arm_disarm_func(bool on_or_off)
{
    ...
    if (uav_state.armed)
    {
        ...
        {
            this->text_info.MessageType = uavcontrol_msgs::TextInfo::WARN;
...
void UAV_controller::timercb_check_px4_location_source(const ros::TimerEvent &e)
{
    ...
    if (px4_params.empty())
    {
        ...
        this->text_info.MessageType = uavcontrol_msgs::TextInfo::INFO;
