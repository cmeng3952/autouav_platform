...
// 【发布】相机(d435i)里程计，用于需要相机坐标的数据消费者
    ...
    // 【新增发布】飞控与任务状态话题
    px4_armed_pub = nh.advertise<std_msgs::Bool>(uav_name + "/prometheus/status/armed", 1, true);
    takeoff_completed_pub = nh.advertise<std_msgs::Bool>(uav_name + "/prometheus/status/takeoff_completed", 1, true);
    landing_completed_pub = nh.advertise<std_msgs::Bool>(uav_name + "/prometheus/status/landing_completed", 1, true);
