...
UAVBasic::UAVBasic(ros::NodeHandle &nh, int id, Communication *communication)
{
    ...
    this->uav_cmd_pub_ = nh.advertise<uavcontrol_msgs::UAVCommand>("/uav" + std::to_string(id) + "/prometheus/command",2);
    // 【发布】mavros接口调用指令(-> uav_control.cpp)
    this->uav_setup_pub_ = nh.advertise<uavcontrol_msgs::UAVSetup>("/uav" + std::to_string(this->robot_id) + "/prometheus/setup", 2);
    // 【订阅】uav控制信息
    this->uav_cmd_sub_ = nh.subscribe("/uav" + std::to_string(id) + "/prometheus/command", 10, &UAVBasic::uavCmdCb, this);
    // 【发布】跟踪目标
    this->uav_target_pub_ = nh.advertise<uavcontrol_msgs::Control>("/uav" + std::to_string(this->robot_id) + "/spirecv/control", 2);
