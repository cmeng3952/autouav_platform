...
SwarmControl::SwarmControl(ros::NodeHandle &nh, Communication *communication, SwarmMode mode, int id, int swarm_num)
{
    ...
    // 【发布】集群控制指令
    this->swarm_command_pub_ = nh.advertise<uavcontrol_msgs::SwarmCommand>("/prometheus/swarm_command", 1000);
    // 【订阅】集群控制指令
    ...
    if (mode == SwarmMode::ONLY_UAV)
    {
        ...
        // 【发布】所有无人机状态
        this->all_uav_state_pub_ = nh.advertise<uavcontrol_msgs::MultiUAVState>("/prometheus/all_uav_state", 1000);
    }
    else if (mode == SwarmMode::ONLY_UGV)
    {
        ...
        // 【发布】所有无人车状态
        this->all_ugv_state_pub_ = nh.advertise<uavcontrol_msgs::MultiUGVState>("/prometheus/all_ugv_state", 1000);
    }
    else
    {
...
{
    // 进行发布
    uavcontrol_msgs::SwarmCommand m_swarm_command;
...
{
    uavcontrol_msgs::MultiUAVState multi_uav_state;
...
{
    uavcontrol_msgs::MultiUGVState multi_ugv_state;
    ...
    {
        uavcontrol_msgs::UGVState ugv_state;
...
void SwarmControl::swarmCmdCb(const uavcontrol_msgs::SwarmCommand::ConstPtr &msg)
{
