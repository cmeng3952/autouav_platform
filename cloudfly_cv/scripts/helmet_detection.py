...
def on_mqtt_message(client, userdata, msg):
    # 控制指令：
    global enable_processing,pipe,cap,shutdown_flag,ai_stream_warmup_frames # 全局变量
    try:
...
def mqtt_publish_detections(mqtt_client,object_counts,boxes,classes,scores,frame,last_mqtt_publish_time):
    ...
    if mqtt_current_time -last_mqtt_publish_time>=cfg.mqtt_publish_interval:
        ...
        #判断识别未戴头盔数量是否达到阈值
        if object_counts['without_helmet']>=cfg.WITHOUT_HELMET_COUNT_THRESHOLD:
            consecutive_alarm_frames += 1
            if consecutive_alarm_frames>=cfg.ALARM_STABLE_FRAMES:
                alarm=True #满足条件，触发报警
        ...
        msg=json.dumps({
            ...
            "ai_colors":cfg.MQTT_HELMET_COLORS,
            "detections":detections,
            "choose_id":3,
            "alarm":alarm,
            "warning_type":"未戴头盔" if alarm else "",
...
def load_config_from_file():
    try:
        with open('helmet_shared_config.json', 'r') as f:
            config = json.load(f)
        # 更新全局配置
        cfg.CONFIDENCE = float(config.get("CONFIDENCE", cfg.CONFIDENCE))
        cfg.WITHOUT_HELMET_COUNT_THRESHOLD=int(config.get("WITHOUT_HELMET_COUNT_THRESHOLD", cfg.WITHOUT_HELMET_COUNT_THRESHOLD))
        cfg.ALERT_FRAME_SAVE_INTERVAL = int(config.get("ALERT_FRAME_SAVE_INTERVAL", cfg.ALERT_FRAME_SAVE_INTERVAL))
    except Exception as e:
        print(f"[CONFIG] 读取 helmet_shared_config.json 失败: {e}，使用默认值")
#类别过滤
def filter_detection_results(boxes, classes, scores, allowed_classes):
    """
    过滤检测结果，只保留允许的类别
    ...
    Args:
        ...
        classes: 类别索引
        scores: 置信度分数
        allowed_classes: 允许的类别名称列表
    
    Returns:
        过滤后的boxes, classes, scores
    """
        ...
        while not shutdown_flag:
            ...
            if processing:
                ...
                if frame_count % cfg.frame_interval == 0:
                    ...
                    result = model.get_result()  # 获取推理结果
                    if result is not None:
                        infer_frame, boxes, classes, scores, _ = result
                        if boxes is not None:    
                            try:
                                # 过滤只保留指定的类别（例如只保留helmet和without_helmet）
                                boxes, classes, scores = filter_detection_results(
                                    boxes, classes, scores,cfg.CLASSES
                                )
                                if boxes is not None:
