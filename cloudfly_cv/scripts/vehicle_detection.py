...
is_in_alarm_state=False #是否处于报警状态
...
def mqtt_publish_detections(mqtt_client,object_counts,boxes,classes,scores,frame,last_mqtt_publish_time):
    global consecutive_alarm_frames,is_in_alarm_state,last_screenshot_time
    ...
    if mqtt_current_time -last_mqtt_publish_time>=cfg.mqtt_publish_interval:
        ...
        if total_count<cfg.VEHICLE_CONGESTION_THRESHOLD:
            level="low"
        elif total_count <=cfg.VEHICLE_CONGESTION_THRESHOLD*2:
            level="medium"
        else:
            level="high" 
        #如果拥堵等级不是低，则报警
        ...
        msg=json.dumps({
            ...
            "ai_colors":cfg.MQTT_VEHICLE_COLORS,
            "detections":detections,
            "choose_id":2,
            "alarm":alarm,
            "congestion_level":level,
            "warning_type":"道路拥堵" if level != "low" else "",
            "warning_imageURL":image_url,
...
def load_config_from_file():
    try:
        with open('vehicle_shared_config.json', 'r') as f:
            config = json.load(f)
        # 更新全局配置
        cfg.CONFIDENCE = float(config.get("CONFIDENCE", cfg.CONFIDENCE))
        cfg.VEHICLE_CONGESTION_THRESHOLD=int(config.get("VEHICLE_CONGESTION_THRESHOLD", cfg.VEHICLE_CONGESTION_THRESHOLD))
        cfg.ALERT_FRAME_SAVE_INTERVAL = int(config.get("ALERT_FRAME_SAVE_INTERVAL", cfg.ALERT_FRAME_SAVE_INTERVAL))
        
        
    except Exception as e:
        print(f"[CONFIG] 读取 vehicle_shared_config.json 失败: {e}，使用默认值")
# 主函数
if __name__ == '__main__':
    try:
        ...
        #初始化参数
        config=cfg.MODEL_CONFIGS["vehicle"]
